...
class MLFCore:
    ...
    def log_conda_environment(reports_output_dir: str):
        ...
        conda_env_filehandler = open(f'{reports_output_dir}/liver_ct_segmentation_conda_environment.yml', 'w')
        subprocess.call(['conda', 'env', 'export', '--name', 'liver_ct_segmentation'], stdout=conda_env_filehandler)
        print('[bold blue]Uploading conda environment report as a run artifact...')
        mlflow.log_artifact(f'{reports_output_dir}/liver_ct_segmentation_conda_environment.yml', artifact_path='reports')

    @staticmethod
    def set_pytorch_random_seeds(seed, num_gpus):
        torch.manual_seed(seed)
        #torch.set_deterministic(True)
        torch.use_deterministic_algorithms(True) # <-- For reasons of new version
        # https://github.com/pytorch/pytorch/issues/58096
        if num_gpus > 0:
